  ...
  try {
    ...
    db.serialize(() => {
      ...
      db.run(`CREATE TABLE IF NOT EXISTS orderInfo (
        ...
        skuId TEXT,
        skuMapperId TEXT,
        status TEXT DEFAULT 'unfinish'
      )`);
      
      // 创建SKU映射表
      db.run(`CREATE TABLE IF NOT EXISTS skuMapper (
        id TEXT PRIMARY KEY,
        sku TEXT,
        url TEXT,
    ...
    db.close();
    console.log(`空数据库已创建: ${dbPath}`);
  } catch (error) {
    console.error('创建空数据库失败:', error);
  }
}
